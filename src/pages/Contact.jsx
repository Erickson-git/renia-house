...
export default function Contact() {
  ...
  return (
    <section className="bg-brown px-5 py-16 text-cream md:py-24">
      <div className="mx-auto max-w-6xl">
        ...
        <div className="grid gap-10 md:grid-cols-2">
          {/* Formulaire WhatsApp + coordonnées */}
          <motion.div
          ...
          >
            <form onSubmit={envoyer} className="space-y-4 rounded-2xl border border-cream/15 bg-cream/5 p-6">
              <div>
                <label className="mb-1 block text-xs uppercase tracking-luxe text-gold-soft">Votre nom</label>
              ...
              <div>
                <label className="mb-1 block text-xs uppercase tracking-luxe text-gold-soft">Votre message</label>
                <textarea value={msg} onChange={(e) => setMsg(e.target.value)} rows="3" placeholder="Ex : je voudrais un tissage samedi…"
                  className="w-full rounded-lg border border-cream/20 bg-brown-700 px-3 py-2.5 text-cream outline-none focus:border-gold-soft" />
              </div>
            ...
            <div className="border-t border-cream/15 pt-5">
              <p className="mb-2 text-sm uppercase tracking-luxe text-gold-soft">Horaires</p>
          ...
          >
            <iframe title="Carte Renia House" src={mapsSrc} className="h-full min-h-[420px] w-full" loading="lazy" referrerPolicy="no-referrer-when-downgrade" />
          </motion.div>
        </div>
      </div>
    </section>
